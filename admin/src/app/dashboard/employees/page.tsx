...
const EMPTY: Omit<Employee, 'id' | 'createdAt'> = {
  ...
  contractType: 'full_time', nekudotZikui: 2.25, grossSalary: 0, notes: '',
};
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl">
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          {[
            ...
            { label: 'שכר ברוטו (₪)', key: 'grossSalary', type: 'number', dir: 'ltr' },
            { label: 'נקודות זיכוי', key: 'nekudotZikui', type: 'number', dir: 'ltr' },
          ].map(({ label, key, type, dir }) => (
            <div key={key}>
              <label className="block text-xs text-gray-500 font-hebrew mb-1">{label}</label>
              <input type={type} value={(form as Record<string, unknown>)[key] as string ?? ''} dir={dir}
                onChange={(e) => set(key as keyof typeof form, type === 'number' ? Number(e.target.value) : e.target.value)}
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
          ...
          <button onClick={async () => { if (!form.name || !form.phone) return; setSaving(true); await onSave(form); setSaving(false); onClose(); }}
            disabled={saving} className="flex items-center gap-2 bg-navy text-white px-4 py-2 rounded-lg text-sm font-hebrew hover:bg-navy/90 disabled:opacity-60">
...
export default function EmployeesPage() {
  const { employees, loading, createEmployee, updateEmployee, toggleStatus } = useEmployees();
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-50 bg-gray-50/50">
                {['שם', 'טלפון', 'אזור', 'חוזה', 'שכר ברוטו', 'סטטוס', ''].map((h) => (
            ...
            <tbody>
              ...
              {filtered.map((emp) => (
                <tr key={emp.id} className={`border-b border-gray-50 hover:bg-gray-50/40 transition-colors ${emp.status === 'inactive' ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-3 py-3.5 text-gray-600" dir="ltr">{emp.grossSalary ? `₪${emp.grossSalary.toLocaleString()}` : '—'}</td>
